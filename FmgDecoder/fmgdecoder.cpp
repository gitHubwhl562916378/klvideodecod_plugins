#include "fmgdecoder.h"
#include <iostream>

Fmgdecoder::~Fmgdecoder()
{
    if(m_mtx){
        m_mtx->lock();
        av_free(m_buffer);
...
bool Fmgdecoder::initsize()
{
    return true;
}

unsigned char *Fmgdecoder::framePtr()
{
    return m_buffer;
}

int Fmgdecoder::fps() const
{
    return m_fps;
}

bool Fmgdecoder::decode(const char* source, std::string &erroStr, std::function<void(AVPixelFormat,unsigned char*,int,int)> frameHandler, std::mutex *mtx)
{
    m_run = true;
    m_mtx = mtx;
